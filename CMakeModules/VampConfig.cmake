...
# ========================
# This module provides a clean, modular interface for VAMP integration with
# OMPL. It handles SIMD configuration, Python setup, and target management
...
function(configure_vamp)
  if(NOT OMPL_BUILD_VAMP)
    set(OMPL_HAVE_VAMP
        FALSE
        CACHE BOOL "Whether VAMP integration is available" FORCE)
    return()
  endif()
  ...
  if(NOT EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/external/vamp/CMakeLists.txt")
    message(
      WARNING
        "VAMP submodule not found. Run 'git submodule update --init --recursive' to initialize it."
    )
    set(OMPL_HAVE_VAMP
        FALSE
        CACHE BOOL "Whether VAMP integration is available" FORCE)
...
function(configure_vamp_build_options)
  ...
  set(VAMP_PORTABLE_BUILD
      ${VAMP_PORTABLE_BUILD}
      CACHE BOOL "Build VAMP with portable SIMD settings" FORCE)

  # Set VAMP Python bindings - use top-level option value
  set(VAMP_BUILD_PYTHON_BINDINGS
      ${VAMP_BUILD_PYTHON_BINDINGS}
      CACHE BOOL "Build VAMP Python bindings" FORCE)

  # Set VAMP-specific options
  set(VAMP_BUILD_CPP_DEMO
      OFF
      CACHE BOOL "Build VAMP C++ Demo Scripts" FORCE)
  set(VAMP_BUILD_OMPL_DEMO
      OFF
      CACHE BOOL "Build VAMP C++ OMPL Integration Demo Scripts" FORCE)
  set(VAMP_OMPL_PATH
      "${CMAKE_CURRENT_SOURCE_DIR}"
      CACHE STRING "OMPL path for VAMP integration" FORCE)
endfunction()

# Setup VAMP SIMD environment
function(setup_vamp_simd_environment)
  # Store current flags to restore later (use cache to persist across function
  # calls)
  set(_VAMP_OLD_CMAKE_C_FLAGS
      "${CMAKE_C_FLAGS}"
      CACHE INTERNAL "Stored C flags before VAMP SIMD setup")
  set(_VAMP_OLD_CMAKE_CXX_FLAGS
      "${CMAKE_CXX_FLAGS}"
      CACHE INTERNAL "Stored CXX flags before VAMP SIMD setup")
  ...
  elseif(CMAKE_SYSTEM_PROCESSOR STREQUAL "aarch64" OR CMAKE_SYSTEM_PROCESSOR
    ...
    message(
      FATAL_ERROR "Unsupported architecture ${CMAKE_SYSTEM_PROCESSOR} for VAMP")
  endif()
  ...
  set(CMAKE_C_FLAGS
      "${CMAKE_C_FLAGS} ${SIMD_FLAGS_STR}"
      CACHE STRING "" FORCE)
  set(CMAKE_CXX_FLAGS
      "${CMAKE_CXX_FLAGS} ${SIMD_FLAGS_STR}"
      CACHE STRING "" FORCE)
...
function(restore_original_flags)
  if(DEFINED _VAMP_OLD_CMAKE_C_FLAGS)
    set(CMAKE_C_FLAGS
        "${_VAMP_OLD_CMAKE_C_FLAGS}"
        CACHE STRING "" FORCE)
  endif()
  if(DEFINED _VAMP_OLD_CMAKE_CXX_FLAGS)
    set(CMAKE_CXX_FLAGS
        "${_VAMP_OLD_CMAKE_CXX_FLAGS}"
        CACHE STRING "" FORCE)
  endif()
endfunction()
