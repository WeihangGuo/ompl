...
class ViserVisualizer:
    ...
    def __init__(
        self, robot_name: str, robot_dimension: int, port: Optional[int] = None
    ):
    ...
    def reset(self):
        ...
        self.urdf_vis = ViserUrdf(
            self.server, self.robot_urdf, root_node_name=f"/{self.robot_name}"
        )
    ...
    def load_mbm_environment(
        ...
        for obj in problem_data.get("cylinder", []):
            ...
            orientation_euler = np.array(
                obj.get("orientation_euler_xyz", [0.0, 0.0, 0.0])
            )
            ...
            if is_box_problem:
                ...
                self.add_box(
                    ...
                    color=color,
                    name=f"/cylinder_as_box_{obj['name']}",
                )
            else:
                radius = obj["radius"] + padding
                length = obj["length"]
    ...
    def _generate_name(self, prefix: str) -> str:
        ...
        count = len(
            [
                k
                for k in self.server.scene._handle_from_node_name.keys()
    ...
    def _update_robot_config(
        self, trajectory_idx: int, gripper_dof: float = 0.0
    ) -> None:
    ...
    def add_sphere(
        ...
        self.server.scene.add_icosphere(
            name=name, position=tuple(position), radius=radius, color=color
        )
    ...
    def visualize_trajectory(self, trajectory: np.ndarray):
        ...
        if trajectory.ndim != 2:
            raise ValueError(
                f"Trajectory must be 2D array, got shape {trajectory.shape}"
            )
    ...
    def play_until_key_pressed(self, key="any", dt=0.1):
        ...
        print(
            f"Visualization running. Press {key if key != 'any' else 'any key'} to stop..."
        )
